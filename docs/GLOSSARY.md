...
| **accept line** | The score at or above which a pair is accepted without review. Each track has its own. | auto-accept threshold, threshold_high, high line | `match_probability_threshold_high`, `match_probability_threshold_high_by_track` | Config → Thresholds, New run, Run detail |
...
| **mixed names** | A cluster that holds more different names, addresses or birth dates than one person could have, so it is really several people. | mixed-name cluster | cluster `status` = `mixed_names` | Cluster review |
