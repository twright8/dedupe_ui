...
| `mixed_names` | its units show more different values of a named column than that column's limit allows. The setting is `max_distinct_values` in `linkage_settings.json`, `{track: [clause, ...]}`; the cluster is held when ANY clause holds, and a track with no entry is not gated this way. A clause is one limit, `{"column": "surname_clean", "count": 3}`, or a conjunction, `{"all": [...]}`, which holds only when EVERY limit in it is over its count. A limit may count several columns together as one value with `columns` instead of `column`. PSC person has four clauses: surname over 3, forename over 3, birth year over 2, and postcode districts over 5 **and** full birth dates over 1 together. Donations names none. The single-limit shape is the match keys' own `max_distinct` guard, so a reader who has met one has met both |
...
Only a handful of columns of `units.parquet` are read — `unit_id`, `unit_size`, `track`, `existing_entity_id`, and the columns `max_distinct_values` names, when it names any. Each limit is measured once however many clauses or tracks name it, and a limit over several columns reads them all. The PSC units file carries sixty-odd.
