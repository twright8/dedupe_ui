...
- Pair operators: `differs` (both present, not equal), `not_equal_or_missing` (see below), `abs_diff_gt` and `abs_diff_gte` (numeric difference, `value`), `similarity_lt` (Jaro-Winkler below `value`), `both_in_and_differ` (`lists`: both values are in the named token lists and they differ), `no_overlap` (the two " | "-joined sets share nothing). Their labels and one-sentence definitions are in `app/vocabulary.VETO_OP`, which is what the UI offers.
- **`not_equal_or_missing` is the one operator a null makes true.** `differs` asks "do these two disagree?", which a missing value cannot answer. `not_equal_or_missing` asks "does this column agree?", and a column nobody filed does not agree. It is what a veto rule needs to say "and nothing else identifies them" — the PSC person rule v3 is "the surnames are clearly different AND the full postcodes do not agree AND the middle names do not agree". Because it is true for most pairs on its own, it is only ever written beside a condition that is not. A column the units do not carry makes it true for every pair, for the same reason.
