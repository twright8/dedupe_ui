...
def test_stage_0_writes_the_records_parquet(tmp_path, input_csv):
    run_dir = tmp_path / "run"
    stats = run_stage_0_load(run_dir=str(run_dir), input_path=str(input_csv))

    parquet = run_dir / "records.parquet"
...
def test_config_version_1_is_seeded_from_the_profile_defaults(db_path, monkeypatch):
    ...
    name_rules = json.loads(config["name_rules"])
    assert any("MR" in rule["pattern"] for rule in name_rules)

    legal_tokens = json.loads(config["legal_tokens"])
    assert legal_tokens["tokens"] == ["LTD", "LIMITED", "PLC", "LLP"]
    ...
    assert json.loads(config["jurisdiction_map"]) == []  # header only
